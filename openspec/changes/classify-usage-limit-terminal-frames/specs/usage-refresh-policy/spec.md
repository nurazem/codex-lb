...
When an upstream stream fails with a rejection that proves the account's usage
limit is spent, the proxy MUST request an immediate usage refresh for the failing
account in addition to marking it rate limited. The trigger is that rejection, not
the literal error code: upstream proves the same condition with the code
`usage_limit_reached` and, on the code-less delivery form, with the message
alone, and the account whose window is spent is owed the same refresh either
way. The refresh MUST run as a tracked background task that
...
- **GIVEN** an upstream stream fails with an envelope carrying no error code whose message asserts the account's usage limit has been reached
