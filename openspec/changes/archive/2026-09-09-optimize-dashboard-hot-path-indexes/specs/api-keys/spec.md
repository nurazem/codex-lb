...
**Reason**: The wider `(api_key_id, requested_at, account_id)` index (`idx_logs_api_key_time_account`) is dropped by migration `20260717_000000_optimize_dashboard_hot_path_indexes` as redundant (never selected by the planner; filter phase served by `idx_logs_api_key_time`). Replaced by "API key 7-day account-cost queries use an index-supported filter phase".

**Migration**: None required for operators; the migration drops the redundant index and the account-cost breakdown continues to use `idx_logs_api_key_time` for its filter phase.
