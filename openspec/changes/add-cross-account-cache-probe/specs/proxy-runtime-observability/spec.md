...
#### Scenario: Generated diagnostic traffic is excluded while archiving stays on

- **GIVEN** the conversation archive is enabled
- **WHEN** an operator diagnostic issues its own upstream calls with generated payloads
- **THEN** no archive record is written for those calls
- **AND** the archive still records ordinary proxied traffic once the diagnostic finishes
