...
- The never-tuned core tunables constantized by `constantize-core-tunables`:
  ...
  the only cap.
- The proactive token-refresh window (8 days, `TOKEN_REFRESH_INTERVAL_DAYS`
  in `app/core/auth/refresh.py`), constantized by
  `constantize-token-refresh-interval`: an account is refreshed on demand
  whenever upstream answers 401 whatever the window says, so the window was
  never a recovery lever, and no deployment ever changed it.
