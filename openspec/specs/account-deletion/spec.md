...
## Purpose
Deleting an account used to detach or delete its entire raw history (usage_history, request_logs) in one transaction while holding the fold-state lock, which on production took minutes, pinned a pool connection, timed out the HTTP client and blocked the fold. This capability moves account deletion off the request path into a bounded background process so the API responds promptly while history is detached in chunks.
## Requirements
